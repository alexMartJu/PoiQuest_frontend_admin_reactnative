...
import { routesEventsStaticStyles, getRoutesEventsStyles } from '@/styles/routes.styles';

const ROUTE_FILTER_OPTIONS: SegmentedButtonOption<EventAdminFilter>[] = [
...
export default function RoutesEventsScreen() {
  const theme = useAppTheme();
  const themed = useMemo(() => getRoutesEventsStyles(theme), [theme]);
  const router = useRouter();
  const [routeFilter, setRouteFilter] = useState<EventAdminFilter>(EventAdminFilter.ACTIVE);
  ...
  const currentQuery =
    routeFilter === EventAdminFilter.PENDING ? pendingEventsQuery : activeEventsQuery;

  const { data, isLoading, isFetching, isFetchingNextPage, hasNextPage, fetchNextPage, error, refetch } =
    currentQuery;
  ...
  const handleEventPress = (uuid: string) => {
    router.push(`/(protected)/(drawer)/routes/${uuid}`);
  };
  ...
  return (
    <View style={[routesEventsStaticStyles.container, themed.container]}>
      <View style={[routesEventsStaticStyles.segmentedSection, themed.segmentedSection]}>
      ...
      <FlatList
        ...
        contentContainerStyle={[routesEventsStaticStyles.listContent, themed.listContent]}
        refreshControl={
          <RefreshControl
        ...
        ListEmptyComponent={
          isLoading ? (
            <View style={routesEventsStaticStyles.emptyContainer}>
            ...
            <View style={routesEventsStaticStyles.emptyContainer}>
              ...
              <Text variant="bodyLarge" style={themed.emptyText}>
                No hay eventos{' '}
                {routeFilter === EventAdminFilter.ACTIVE ? 'activos' : 'pendientes'}
              </Text>
            </View>
          )
        }
